...
dataset1 = table2array(readtable("R1_E20_max - Copy.xlsx")); columnNames1 = readtable("R1_E20_max - Copy.xlsx").Properties.VariableNames;      % Turn table to array for compatibility with functions and better visualization
...
X_train = X_all(indices(1:62), :);     % Split the data into training (0.75%) and testing (0.25) sets
X_test = X_all(indices(63:end), :);
y_train = y_all(indices(1:62), :);
y_test = y_all(indices(63:end), :);
...
finaltree = fitrtree(X_train, y_train, 'MaxNumSplits', tuned_node,'MinLeafSize', min_leaf_size);
