...
dataset1 = table2array(readtable("R3_E10.xlsx")); columnNames1 = readtable("R3_E10.xlsx").Properties.VariableNames;      % Turn table to array for compatibility with functions and better visualization
dataset3 = table2array(readtable("R1_E10.xlsx")); columnNames3 = readtable("R1_E10.xlsx").Properties.VariableNames;
...
X_train = X_all(indices(1:571), :);     % Split the data into training (0.75%) and testing (0.25) sets
X_test = X_all(indices(572:end), :);
y_train = y_all(indices(1:571), :);
y_test = y_all(indices(572:end), :);
