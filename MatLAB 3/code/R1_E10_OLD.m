...
dataset1 = table2array(Region1); columnNames1 = Region1.Properties.VariableNames;      % Turn table to array for compatibility with functions and better visualization
dataset3 = table2array(Region3); columnNames3 = Region3.Properties.VariableNames;

corr_matrix = corrcoef(dataset1, 'rows', 'complete');
...
figure; plot_corr(dataset1);
figure; gplotmatrix(dataset1, [], [], "k", ".", [], 'on', 'none', columnNames1, columnNames1);
...
X_train = X_all(indices(1:669), :);     % Split the data into training (0.75%) and testing (0.25) sets
X_test = X_all(indices(670:end), :);
y_train = y_all(indices(1:669), :);
y_test = y_all(indices(670:end), :);
...
max_num_splits = 15;
min_leaf_size = 20; % higher stops model from overfitting
...
hold on;
plot([tuned_nodes, tuned_nodes], [0, 5], 'r--');
...
importance = finaltree.predictorImportance();
twi_importance = round(importance(1), 2);
s_importance = round(importance(2), 2);
id_importance = round(importance(3), 2);
dfc_importance = round(importance(4), 2);
fprintf('TWI Importance = %.2f, S Importance = %.2f, ID Importance = %.2f, DFC Importance = %.2f\n', twi_importance, s_importance, id_importance, dfc_importance);
